 ...
 *
 * Shows every community and squad as a tile, in the same pin → recency order
 * as the top bar, so the user can jump anywhere with one tap. Tiles can be
 * pinned (★) to keep them ahead of recency. Rendered between the top bar and
 * the bottom user panel — it never covers the user panel.
 */
...
export default function MobileNavGrid({ onSelectCommunity, onSelectSquad, onClose }: Props): React.JSX.Element {
  ...
  const ordered = orderByRecency(tiles, lastUsed, pinned);
  ...
  return (
    <div style={g.wrap}>
      ...
      <div style={g.grid}>
        ...
        {ordered.length === 0 && <div style={g.empty}>No communities or squads yet</div>}
      </div>
    </div>
  );
}
...
const g = {
  ...
  grid: { flex: 1, minHeight: 0, overflowY: 'auto' as const, display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(76px, 1fr))', gap: '14px 8px', padding: '12px 12px calc(16px + var(--safe-bottom))' } as React.CSSProperties,
  ...
  empty: { gridColumn: '1 / -1', textAlign: 'center' as const, color: 'var(--color-text-muted)', fontSize: '13px', padding: '24px 0' } as React.CSSProperties,
} as const;
